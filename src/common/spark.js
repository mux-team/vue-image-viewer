...
(function () {
    ...
    var timing = {
        ...
        'mag-ease-in-out': 'cubic-bezier(0.4, 0, 0.2, 1)'
    };
    ...
    Spark.addTimeType = function (name, value){

        if(timing[name] && timing[name] !== value) {
            throw new Error(name + ' type is already registered');
        }
        else {
            timing[name] = value;
        }
    ...
    Spark.css3 = function (dom, property, duration, ease, delay, cb) {
        ...
        if (dom && property) {
            ...
            if (typeof property === 'string') {
                ...
                cssValues[animationName] = dom.style[animationName] + (dom.style[animationName] ? ', ' : '') + name;
